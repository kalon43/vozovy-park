...
export class NakladniVuz extends Vozidlo {
  ...
  private _nosnostTun: number = 1;

  /** Aktuální náklad v tunách – ovlivňuje výpočet spotřeby */
  private _aktualniNakladTun: number;
  ...
  get nosnostTun(): number { return this._nosnostTun; }
  get aktualniNakladTun(): number { return this._aktualniNakladTun; }

  /** Setter – nosnost musí být kladné číslo */
  set nosnostTun(hodnota: number) {
    if (hodnota <= 0) {
   ...
   */
  set aktualniNakladTun(hodnota: number) {
    if (hodnota < 0) {
